...
const writeFileAndSend = async (findObj) => {
    ...
    const originData = await fs.readFileSync('./data.json', { encoding: 'utf8', flag: 'r' })
    ...
    if (!(lastData.type === findObj.lottery && lastData.count === findObj.rank && lastData.game === findObj.game)) {
        axios.post(sendUrl, sendInfo)
        await fs.writeFileSync('./data.json', JSON.stringify(overArr, null, 2));
        console.log(info);
    }
}
